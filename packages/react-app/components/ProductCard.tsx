...
const ProductCard: React.FC<ProductCardProps> = ({ sneaker, index }) => {
  ...
  // Generate random rating - Can possibly be used
  // to rank sellers on the platform
   const generateRandomRating = () => {
    return Math.floor(Math.random() * 3) + 3; // Generates a random number between 3 and 5
  };
    ...
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      ...
      <div className="px-5 pb-5">
        ...
        <div className="flex items-center mt-2.5 mb-5">
          <div className="flex items-center space-x-1 rtl:space-x-reverse">{renderStarRating()}</div>
          <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ms-3">
            {generateRandomRating()} {/* Hardcoded rating value for now */}
          </span>
        </div>
